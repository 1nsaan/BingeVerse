...
const WatchPage = () => {
  ...
  useEffect(() => {
    const getSimilarContent = async () => {
      try {
        const res = await axios.get(`/api/v1/${contentType}/${id}/similiar`);
        setSimilarContent(res.data.similiar);
      } catch (error) {
        if (error.message.includes("404")) {
          setSimilarContent([]);
        }
  ...
  useEffect(() => {
    const getContentDetails = async () => {
      try {
        ...
        setHasPurchased(userPurchased.includes(id));
      } catch (error) {
        if (error.message.includes("404")) {
          setContent(null);
        }
    ...
    <div className='bg-black min-h-screen text-white'>
      <div className='mx-auto container px-4 py-8 h-full'>
        ...
        <div className='aspect-video mb-8 p-2 sm:px-10 md:px-32'>
          {/* Conditional rendering based on watchMovie state */}
          {!watchMovie ? (
            <div className="" >
              {trailers.length > 0 ? (
                <ReactPlayer
            ...
            <ReactPlayer
              ...
              height={"70vh"}
              className='mx-auto overflow-hidden rounded-lg'
              url={`https://www.example.com/path-to-your-movie.mp4`} // Replace with actual movie URL
            />
          )}
        </div>
